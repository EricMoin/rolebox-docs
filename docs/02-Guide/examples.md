...
所有示例位于 [rolebox/examples](https://github.com/mgdream/rolebox/tree/main/examples)。
...
::: tip 最佳学习路径
从 **code-reviewer** 开始（10 行 YAML，零子代理），然后尝试 **team-lead**（引入子代理 dispatch），最后探索 **review-team** 系列（协作图与终止条件）。每个示例都在前一个的基础上增加一个核心概念。所有示例的完整代码在 [rolebox/examples](https://github.com/mgdream/rolebox/tree/main/examples)。
:::
